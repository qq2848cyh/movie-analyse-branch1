...
from application.utils.db_manager import DBManager
from application.utils.new_movie_manager import NewMovieManager
from application.utils.echarts_visualizer import EChartsVisualizer
from application.services.network_analyzer import NetworkAnalyzer
from application.services.sentiment_analyzer import SentimentAnalyzer
from application.services.sentiment_dl import train_bilstm_attn, get_attention_visualization
...
os.environ["HF_HUB_CACHE"] = os.path.join(root_dir, "data", "hf_cache")
...
def _warmup_thread():
    ...
    try:
        ...
        try:
            from application.services.bigdata_recommend_engine import train_tfidf, train_svd, train_sbert
            train_tfidf()
...
def _recommend_tfidf(movie_name):
    from application.services.bigdata_recommend_engine import train_tfidf, _get_stop_words
    import jieba
    import numpy as np
    data = train_tfidf()
...
def _recommend_svd(movie_name):
    from application.services.bigdata_recommend_engine import train_svd, train_tfidf
    import numpy as np
...
def _recommend_sbert(movie_name):
    import numpy as np
    from application.services.bigdata_recommend_engine import train_sbert
    data = train_sbert()
    nn_index = pickle.loads(data["nn_index"])
    movies = data["movies"]
...
def api_movie_detail():
    ...
    try:
        from application.services.bigdata_recommend_engine import _load_unified_movies
...
def api_export_results():
    ...
    from application.services.bigdata_recommend_engine import train_tfidf
    td = train_tfidf()
    export.update({
